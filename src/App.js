...
function App() {
    ...
    // Initialize utility functions for new criteria and clean up removed ones
    useEffect(() => {
        setUtilityFunctions(prev => {
            const newUtilityFunctions = {};
            
            // Add utility functions for existing criteria
            criteria.forEach(criterion => {
                if (prev[criterion.id]) {
                    // Keep existing utility function
                    newUtilityFunctions[criterion.id] = prev[criterion.id];
                } else {
                    // Create new utility function for new criterion
                    newUtilityFunctions[criterion.id] = {
            ...
            return newUtilityFunctions;
        });
    }, [criteria]);

    // Update pairwise matrix when criteria change
    useEffect(() => {
        const numCriteria = criteria.length;
        if (numCriteria > 0) {
            setPairwiseMatrix(prev => {
                // If matrix size matches, keep existing values
                if (prev.length === numCriteria) {
                    return prev;
                }
                
                // Create new matrix with appropriate size
                const newMatrix = Array(numCriteria).fill(0).map(() => Array(numCriteria).fill(0));
                ...
                for (let i = 0; i < Math.min(prev.length, numCriteria); i++) {
                    for (let j = 0; j < Math.min(prev.length, numCriteria); j++) {
                        if (i !== j) {
                            newMatrix[i][j] = prev[i]?.[j] || 1;
                        }
                    }
                }
                
                // Fill remaining cells with default values
                for (let i = 0; i < numCriteria; i++) {
                    for (let j = 0; j < numCriteria; j++) {
